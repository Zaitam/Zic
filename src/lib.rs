...
pub const RAW_PIXEL_BYTES_SIZE: u32 = 25; // The size that a raw pixel will have in hex
...
pub fn encode(mut data: Vec<Pixel>, image_desc: ImageDesc) {
    let huffman = build_huffman_tree(&data);
    ...
    match huffman.get(&previous_px) {
        None => {
            buffer.push_bits(vec![
                EncodedBit { data: *RAW_PIXEL_TAG, bit_len: 3 },
                EncodedBit { data: previous_px.r, bit_len: 8 },
                EncodedBit { data: previous_px.g, bit_len: 8 },
                EncodedBit { data: previous_px.b, bit_len: 8 },
            ])
        }
        Some(i) => {
            buffer.push_bits(vec![ EncodedBit { data: *HUFF_CODE_TAG, bit_len: 1 }, EncodedBit { data: i, bit_len: 1 } ])
        } //TODO: Not checking if first px is smaller than 25 bits
    };
    data.remove(0);
    ...
    for (i, px) in data.iter().enumerate() {
        ...
        match huffman_code {
            Some(i) => {
                if diff_to_prev_pixel != 0 && buffer > 255 {
